...
canonical_url: https://wso2.com/api-platform/docs/ai-gateway/1.2.0/setup-and-deployment/immutable-gateway/
md_url: https://wso2.com/api-platform/docs/ai-gateway/1.2.0/setup-and-deployment/immutable-gateway.md
tags:
  - ai-gateway
  - deployment
  - gitops
...
!!! note "Toggling immutable mode from the environment"
    ...
    Then set `APIP_GW_IMMUTABLE_GATEWAY_ENABLED=true`. See [Gateway Configuration and Environment Interpolation](./configuration.md).
...
Use `| redact` for sensitive values to hide them from config dumps. A `| default "value"` pipe is available for fallback values. See [Gateway Artifact Templating](../../../api-gateway/1.2.0/setup/artifact-templating.md) for the full function reference.
