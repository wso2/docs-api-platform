...
canonical_url: https://wso2.com/api-platform/docs/ai-gateway/next/guardrails/guardrails-catalogue/
md_url: https://wso2.com/api-platform/docs/ai-gateway/next/guardrails/guardrails-catalogue.md
tags:
  - ai-gateway
  - guardrails
  - llm-proxy
...
A guardrail is a gateway policy, so you add or update one by editing the gateway's `build.yaml` and rebuilding the image with the `ap` CLI. See [Add or Update Policies in a Gateway](../../../api-gateway/next/policies/add-or-update-policies.md).

## Custom guardrails

To write your own guardrail policy in Go and build it into the gateway image, see [Writing a Custom Policy](../../../api-gateway/next/policies/custom-policies/writing-a-custom-policy.md).
