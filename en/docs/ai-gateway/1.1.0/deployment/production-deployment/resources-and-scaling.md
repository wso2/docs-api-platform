...
canonical_url: https://wso2.com/api-platform/docs/ai-gateway/1.1.0/deployment/production-deployment/resources-and-scaling/
md_url: https://wso2.com/api-platform/docs/ai-gateway/1.1.0/deployment/production-deployment/resources-and-scaling.md
tags:
  - ai-gateway
  - production
  - kubernetes
...
The allocations below match the ones used in the [AI Gateway performance tests](../../performance/overview.md), so the published throughput and latency figures describe what these settings deliver.
...
- **Payload size.** LLM request and response bodies are far larger than typical representational state transfer (REST) payloads, and body-inspecting policies buffer them in the runtime's memory. The ceiling multiplies by concurrency, so size the memory limit against both.
