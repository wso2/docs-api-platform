...
Encrypted secret management — create, rotate, and delete organization-scoped secrets referenced via {% raw %}`{{ secret "name" }}`{% endraw %} placeholders
...
so all {% raw %}`{{ secret "handle" }}`{% endraw %} placeholder references across resources remain valid
