...
7.  Take a look at the generated CNAME target value displayed and create a DNS record associating the domain name to the CNAME target value with your DNS provider.
    ...
    ![Add a Custom Domain wizard showing domain name, environment selector, and generated CNAME record with target value](../../assets/img/administer/settings/cname-target-value.png)
               ...
               To construct a certificate chain with a root certificate, you must organize and combine the certificates in the correct sequence. A typical certificate chain consists of the following:
                ...
                Follow these steps to construct the certificate chain:
                    ...
                    5. Once the certificate chain is verified, upload it via the API Platform Console:
                        ![Add a Custom Domain step 2 showing Import Certificate option with TLS Certificate, TLS Key File, and Certificate Chain File upload fields](../../assets/img/administer/settings/upload-certificate-chain.png)
...
![Active Domains tab listing newtest.saasorg.work.gd domain with API type, Development environment, and Managed by Bijira certificate status](../../assets/img/administer/settings/active-domains.png)
...
4. Click the **URL Settings** tab. This displays the active deployments of the component across different environments and indicates whether a custom URL is configured. If an active custom domain is available to configure a custom URL for a component in a specific environment, the **Edit URL Mapping** icon in the corresponding **Action** column becomes enabled.
   ...
   ![URL Settings tab listing Production and Development deployments with Not Requested approval status and Edit URL Mapping icon](../../assets/img/administer/settings/active-deployments.png)
...
5. To configure a custom URL for a component in a specific environment, click the **Edit URL Mapping** icon under the **Action** column corresponding to the respective environment. This opens the **URL Settings** dialog, where you can specify values to add to a custom URL.
   ...
   ![URL Settings dialog showing Environment, Default URL, Domain dropdown, Path, and Customized URL fields with Deploy button](../../assets/img/administer/settings/url-settings.png){: style="height:500px;width:380px"}
...
7. Click **Deploy**. This creates the custom URL mapping, which you can see under the **URL Settings** tab. The custom URL will be in the **Approved** state.
   ...
   ![URL Settings tab showing Development row with custom URL and Approved status badge](../../assets/img/administer/settings/approved-custom-url.png)

   Once deployed, both the custom URL and the default URL can be used to access the component.

   ![API proxy overview showing Development environment deployed with active custom URL displayed](../../assets/img/administer/settings/custom-url.png)
