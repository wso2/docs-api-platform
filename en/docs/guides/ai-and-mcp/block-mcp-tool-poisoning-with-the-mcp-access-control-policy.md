...
MCP tool poisoning is a prompt-injection technique where a malicious or compromised MCP server embeds malicious instructions in tool metadata, such as a tool's `description` field. An MCP client may pass that metadata to an LLM, and a person reviewing a tool list in a UI can easily miss wording that a model still reads. This guide explains what tool poisoning is, why it's a real risk for any agent connected to an MCP server you don't fully control, and how attaching the **MCP Access Control** policy to an **MCP Proxy** in WSO2 API Platform reduces your exposure to it.
...
- **It can attempt to trigger data exfiltration if the agent has the means.** A poisoned description can try to persuade the model to use another tool the agent already has access to — one that reads local files or environment variables, for example — and then disclose that content through some other capability the agent is already permitted to use. The description itself can't read anything; it can only try to talk the model into using capabilities and permissions the agent already has.
- **It can lead to unintended use of other tools.** An agent that already has several tools available can, depending on the client or host implementation, be steered by one poisoned description into invoking another tool for a purpose the person operating it never asked for, within the agent's existing permissions. This isn't a case of the injection bypassing an authorization boundary; it's the model being manipulated into using a capability it was already authorized to use.
...
!!! tip "Testing over plain HTTP avoids a self-signed certificate"
...
The URL follows the pattern `https://<gateway-host>/<proxy-context>/mcp`. Per the tip in Step 2, use the gateway's plain HTTP port instead if you'd rather avoid the self-signed certificate when calling the proxy directly.
...
| `curl` fails with a TLS or certificate error | You're calling the gateway's `https://` port with its self-signed certificate. Use the plain HTTP port instead (see the tip in Step 2), or add `-k` to the `curl` command. |
