...
canonical_url: https://wso2.com/api-platform/docs/api-portal/next/deployment/security-hardening/
md_url: https://wso2.com/api-platform/docs/api-portal/next/deployment/security-hardening.md
tags:
  - cloud
  - api-portal
  - deployment
...
=== "Virtual machine"
    ...
    `scripts/setup.sh` writes them as files under `resources/`, which the compose file mounts into the container. They are read through {% raw %}`{{ file }}`{% endraw %} tokens rather than environment variables deliberately, so they never appear in `docker inspect` or a process environment dump:
