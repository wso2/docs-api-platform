...
description: "Configure Carbon logs, audit logs, and other log types generated by WSO2 API Manager, including per-component log levels in log4j2.properties."
canonical_url: https://wso2.com/api-platform/docs/api-manager/3.0.0/administer/product-administration/monitoring/logging/setting-up-logging/
md_url: https://wso2.com/api-platform/docs/api-manager/3.0.0/administer/product-administration/monitoring/logging/setting-up-logging.md
tags:
  - api-manager
  - administer
...
Following is the default configuration for carbon logs and the default values can be changed by manually updating log4j2.properties file.
   ...
   The log growth of carbon logs can be managed by the configurations discussed in [Managing log growth](managing-log-growth) guide.
...
##HTTP Access Logs
HTTP requests/responses are logged in access logs to monitor the activities related to an application's usage. Please see [Monitoring HTTP Access Logs](monitoring-http-access-logs) for instructions on how to configure and use HTTP access logs in WSO2 API Manager.

##Audit Logs
Audit logs are used for tracking the sequence of actions that affect a particular task carried out on the server. Please see [Monitoring Audit Logs](monitoring-audit-logs) for instructions on how to configure and use audit logs in WSO2 API Manager.
