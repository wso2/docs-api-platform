...
description: "Integrate an external XACML entitlement server with API Manager to enforce role-based access control on exposed APIs."
canonical_url: https://wso2.com/api-platform/docs/api-manager/3.0.0/learn/api-security/authorization/role-based-access-control-using-xacml/
md_url: https://wso2.com/api-platform/docs/api-manager/3.0.0/learn/api-security/authorization/role-based-access-control-using-xacml.md
tags:
  - api-manager
  - learn
  - api-security
...
1.  Let’s start by creating the required users. First, you need to link both the API Manager and the Identity Server to the same user store in order to share users, roles and other related information. This can be done by linking the API manager with the LDAP user store within WSO2 Identity Server. For more information, see [Configuring an external LDAP or Active Directory Userstore](../../../administer/product-administration/managing-users-and-roles/managing-user-stores/configure-primary-user-store/configuring-a-read-write-ldap-user-store) . For this you can create a read write LDAP user store.
    ...
    !!! tip
        In an actual deployment, both these servers can [share the user store](../../extensions/saml2-sso/configuring-identity-server-as-idp-for-sso#sharing-the-user-store) of your organization.
...
14. Log in to the API Publisher and [create an API](../../design-api/create-api/create-a-rest-api).
15. Attach the custom sequence to the inflow of the message as shown below.
    ![](../../../assets/img/learn/attach-the-custom-sequence.png)
