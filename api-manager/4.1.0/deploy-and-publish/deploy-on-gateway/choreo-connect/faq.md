...
description: "Find answers to common questions about Choreo Connect architecture, deployment, licensing, and troubleshooting."
canonical_url: https://wso2.com/api-platform/docs/api-manager/4.1.0/deploy-and-publish/deploy-on-gateway/choreo-connect/faq/
md_url: https://wso2.com/api-platform/docs/api-manager/4.1.0/deploy-and-publish/deploy-on-gateway/choreo-connect/faq.md
tags:
  - api-manager
  - deploy-and-publish
  - deploy-on-gateway
...
* [Choreo Connect standalone Gateway mode with apictl](concepts/as-a-standalone-gateway.md)
    ...
    * [Git integration with source watcher](git-integration.md)
...
You can write a [custom filter](extensions/custom-filters.md) in the Enforcer component or use a separate [interceptor service](message-transformation/interceptor-microservice/interceptor-microservice.md). Also, you can perform custom request validation using the [Open Policy Agent (OPA)](../../../design/api-security/opa-validation/custom-opa-policy-for-choreo-connect.md).
...
Scope validation, subscription validation, custom claims mapping and Open Policy Agent (OPA) validation. Refer to [API Authorization](security/api-authorization/scope-validation.md) for more information.
...
You can change the log format only in the Adapter and Enforcer. Currently, Choreo Connect supports plain text format (default) and JSON formatted logs only. Refer to the [Logs configuration](configurations/configure-logs-overview.md) for more information.

#### How can I inspect HTTP headers and payloads related to a request?

Enable Router trace logs. Refer to the [Router logs configuration](configurations/configure-logs-router.md) for more information. 
