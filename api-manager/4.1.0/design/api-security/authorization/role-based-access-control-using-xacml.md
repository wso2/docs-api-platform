...
description: "Explains how to integrate an external XACML entitlement server with API Manager for role-based access control."
canonical_url: https://wso2.com/api-platform/docs/api-manager/4.1.0/design/api-security/authorization/role-based-access-control-using-xacml/
md_url: https://wso2.com/api-platform/docs/api-manager/4.1.0/design/api-security/authorization/role-based-access-control-using-xacml.md
tags:
  - api-manager
  - design
  - api-security
...
1.  Let’s start by creating the required users. First, you need to link both the API Manager and the Identity Server to the same user store in order to share users, roles and other related information. This can be done by linking the API manager with the LDAP user store within WSO2 Identity Server. For more information, see [Configuring an external LDAP or Active Directory Userstore](../../../administer/managing-users-and-roles/managing-user-stores/configure-primary-user-store/configuring-a-read-write-ldap-user-store.md) . For this you can create a read write LDAP user store.
    ...
    !!! tip
        In an actual deployment, both these servers can [share the user store](../../../reference/customize-product/extending-api-manager/saml2-sso/configuring-external-idp-through-identity-server-for-sso.md) of your organization.
...
14. Log in to the API Publisher and [create an API](../../create-api/create-rest-api/create-a-rest-api.md).
15. Attach the custom sequence to the inflow of the message as shown below.
    ![](../../../assets/img/learn/attach-custom-in-sequence.png)
