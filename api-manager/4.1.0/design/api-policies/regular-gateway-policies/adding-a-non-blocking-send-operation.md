...
description: "Configure a non-blocking Send mediator policy to transfer large files to a VFS endpoint without blocking."
canonical_url: https://wso2.com/api-platform/docs/api-manager/4.1.0/design/api-policies/regular-gateway-policies/adding-a-non-blocking-send-operation/
md_url: https://wso2.com/api-platform/docs/api-manager/4.1.0/design/api-policies/regular-gateway-policies/adding-a-non-blocking-send-operation.md
tags:
  - api-manager
  - design
  - api-policies
...
Let's assume that the policy file that you created by considering the example above is `addNonBlockingSend.xml`. We will next create a policy using this policy file. You can refer to [Create a Policy](../create-policy.md) to learn more about how to achieve this. When creating the policy, make sure to only select the `Request` flow when filling out the general details of the create policy form. Thus created policy can then be attached to the request flow of any API operation (refer [Attach Policies](../attach-policy.md) for further details).