...
description: "Reference for Salesforce REST connector operations and connection parameters, including authentication options."
canonical_url: https://wso2.com/api-platform/docs/api-manager/4.1.0/reference/connectors/salesforce-connectors/sf-rest-connector-config/
md_url: https://wso2.com/api-platform/docs/api-manager/4.1.0/reference/connectors/salesforce-connectors/sf-rest-connector-config.md
tags:
  - api-manager
  - reference
  - connectors
...
To use the Salesforce REST connector, add a connection before carrying out any other Salesforce REST operations. In previous versions, the connector was initialized using the init operation. In the latest connector versions, the init operation, which is used to initiate the connector, is created as a local entry and then referred from Integration Studio itself.

Once you add an operation by dragging it onto the canvas, you can create a connection from the properties window by clicking on the '+' icon. In the popup window, the following parameters must be provided.
