...
description: "Create an application in the Developer Portal to subscribe to APIs and generate consumer key and secret pairs."
canonical_url: https://wso2.com/api-platform/docs/api-manager/4.1.0/consume/manage-application/create-application/
md_url: https://wso2.com/api-platform/docs/api-manager/4.1.0/consume/manage-application/create-application.md
tags:
  - api-manager
  - consume
  - manage-application
...
WSO2 API Manager comes with a pre-created default application, which allows unlimited access by default. Default application creation can be disabled by adding the configuration `"DisableDefaultApplicationCreation" : true` in to the `Advanced Configurations` under `Settings` of the `admin` portal. You can also [create](create-application.md#create-a-new-application) your own.
