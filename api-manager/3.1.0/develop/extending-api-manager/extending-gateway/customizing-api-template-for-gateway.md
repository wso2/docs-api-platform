...
description: "Modify the gateway's Velocity API template to conditionally engage custom handlers and use template variables."
canonical_url: https://wso2.com/api-platform/docs/api-manager/3.1.0/develop/extending-api-manager/extending-gateway/customizing-api-template-for-gateway/
md_url: https://wso2.com/api-platform/docs/api-manager/3.1.0/develop/extending-api-manager/extending-gateway/customizing-api-template-for-gateway.md
tags:
  - api-manager
  - develop
...
2.  Add following definition to engage the custom handler.
    ...
    **Example 2** - Switching the custom handler with one of the default handlers
    ...
    !!!Info
        ...
        If you are using a [distributed API Manager deployment](../../../install-and-setup/setup/distributed-deployment/understanding-the-distributed-deployment-of-wso2-api-m.md) (i.e., Publisher, Devportal, Gateway and Key Manager components are running on separate JVMs), edit the template in the Publisher node. 
     
 3. Save the changes.
 
 4. Add `custom_authentication=true` as an [additional property](../../../learn/design-api/create-api/adding-custom-properties-to-apis.md) for those APIs which you need to enable the custom handler.
