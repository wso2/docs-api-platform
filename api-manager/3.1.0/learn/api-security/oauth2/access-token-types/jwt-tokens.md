...
description: "Secure APIs using self-contained JWT access tokens and create a JWT-type application in the Developer Portal."
canonical_url: https://wso2.com/api-platform/docs/api-manager/3.1.0/learn/api-security/oauth2/access-token-types/jwt-tokens/
md_url: https://wso2.com/api-platform/docs/api-manager/3.1.0/learn/api-security/oauth2/access-token-types/jwt-tokens.md
tags:
  - api-manager
  - learn
  - api-security
...
![](../../../../assets/img/learn/external-jwt-signature-validation.png)
...
1. The JWT needs to have the consumer key of the generated OAuth client application. By default, the API Manager expects the consumer key to be in the `azp` claim or the `consumerKey` claim. If the consumer key comes in a different claim than these 2, the user can map that claim into one of the above 2 claims as per [Transform JWT Claims](#jwt-claim-transformation) section. This step depends on the preferred external Key Manager.

2. The consumer key needs to be mapped to the application. For that, you can use [Provisioning Out-of-Band OAuth2 Clients](../../../../learn/api-security/oauth2/provisioning-out-of-band-oauth-clients.md#provisioning-out-of-band-oauth2-clients).

3. . Navigate to the `deployment.toml` under the `<API-M_HOME>/repository/conf/` folder.

4. Add the **subscription_validation_via_km** configuration under the `[apim.jwt_authenitcation]`.
  ```
