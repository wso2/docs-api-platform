...
canonical_url: https://wso2.com/api-platform/docs/api-manager/4.0.0/integrate/develop/creating-unit-test-suite/
md_url: https://wso2.com/api-platform/docs/api-manager/4.0.0/integrate/develop/creating-unit-test-suite.md
tags:
  - api-manager
  - integrate
  - develop
...
- Test [mediation sequences](creating-artifacts/creating-reusable-sequences.md), [proxy services](creating-artifacts/creating-a-proxy-service.md), and [REST apis](creating-artifacts/creating-an-api.md) with multiple test cases
- Test the artifacts with [registry resources](creating-artifacts/creating-registry-resources.md).
- Test the artifacts with [Connectors](creating-artifacts/adding-connectors.md).

    !!! Note
        [Scheduled Tasks](creating-artifacts/creating-scheduled-task.md) are not supported by the Unit Testing framework.
...
3.  Right-click the **test** folder, which is parallel to the **src** folder and go to **New** -> **Unit Test Suite** as shown below.
    ...
    ![Create Unit Test Suite](../../assets/img/integrate/create_project/synapse_unit_test/create-test-suite.png)

    The **New Unit Test Suite** wizard opens.
    
4.  Select **Create a New Unit Test Suite** and click **Next**.

    ![Select Create Method](../../assets/img/integrate/create_project/synapse_unit_test/select-create-method.png)
...
5.  Specify a name for the unit test suite. Then, select the artifact file that you want to test from the file list and click **Next**. 
    ...
    ![Fill Unit Test Suite Details](../../assets/img/integrate/create_project/synapse_unit_test/select-main-artifact.png)
    
6.  Select the supporting artifacts from the list as shown below and click **Next**.

    ![Select Supportive Artifacts](../../assets/img/integrate/create_project/synapse_unit_test/select-supportives.png)
...
7.  You can use a mock service to simulate the actual endpoint. If you have an already created Mock Service, select the mock service files from the list as shown below. You can also [create a new Mock Service](#create-mock-service) for this purpose.
    ...
    ![Select Mock Services](../../assets/img/integrate/create_project/synapse_unit_test/select-mock-services.png)
...
1.  Open Unit Test Suite from the project explorer. You can use either the design view or the source view to update the unit test suite.
    ...
    ![Unit Test Form](../../assets/img/integrate/create_project/synapse_unit_test/unit-test-form.png)
...
2.  In design view, click the '<b>+</b>' button under the **Test Artifact, Test Cases and Assertion Details** section to add a new **test case** to the unit test suite. 
    ...
    ![Add Test Case](../../assets/img/integrate/create_project/synapse_unit_test/add-test-case.png)
...
3.  Enter the following information:
    ...
    3.  In the **Assertions** section, you can add multiple assertion belonging to two types: **AssertEquals** check the whether the mediated result and expected values are equal. **AssertNotNull** checks whether the mediated result is not null.
        ...
        ![Add Assertions](../../assets/img/integrate/create_project/synapse_unit_test/add-assertion.png)
...
1.  Right-click the **test** directory and click **Run Unit Test** to run all the unit test suites at once. Alternatively, right-click the particular unit test suite and click **Run Unit Test** to run a selected unit test suite.
    ...
    ![Run Unit Test Suite](../../assets/img/integrate/create_project/synapse_unit_test/run-test.png)
...
2.  Select the specific unit testing server (embedded server or remote server) to run the tests.
    ...
    ![Run Configuration](../../assets/img/integrate/create_project/synapse_unit_test/run-configuration.png)
...
3. Click **Run** to start the unit test. It will start the unit testing server in the console and prints the summary report for the given unit test suite(s) using the response from the unit testing server. 
    ...
    ![Output Console](../../assets/img/integrate/create_project/synapse_unit_test/console-log.png)    
...
2.  Right-click the **test** folder parallel to the **src** folder, and go to **New** -> **Mock Service** as shown below. 
    ...
    ![Create Mock Service](../../assets/img/integrate/create_project/synapse_unit_test/create-mock.png) 
    
3.  Select **Create a New Mock Service** and click **Next**.

    ![Select Create Mock Service Method](../../assets/img/integrate/create_project/synapse_unit_test/select-mock-method.png) 
    
4.  In the **Create a new Mock Service** page, enter the following details:
    
    ![Mock Service Details](../../assets/img/integrate/create_project/synapse_unit_test/mock-details.png) 
...
6.  Fill the **Expected Request to the Mock Service Resource** section if you want to mock an endpoint based on the coming request headers or payload.
    ...
    ![Mock Service Resource Request Details](../../assets/img/integrate/create_project/synapse_unit_test/resource-request.png) 
...
7.  Fill the **Response Send Out from the Mock Service Resource** section to get a response from the service.
    ...
    ![Mock Service Resource Response Details](../../assets/img/integrate/create_project/synapse_unit_test/resource-response.png) 
    ...
    !!! Note 
        ...
        ![Mock Service with one sub context](../../assets/img/integrate/create_project/synapse_unit_test/mockservice-context-sample.png) 
        ...
        ![Mock Service with additional sub Contexts](../../assets/img/integrate/create_project/synapse_unit_test/mockservice-subcontext-sample.png) 
...
![Mock Service Form](../../assets/img/integrate/create_project/synapse_unit_test/mock-service-form.png)     
