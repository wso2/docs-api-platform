...
- Choreo Connect is run as a standalone Gateway and **APIM Publisher** is set as a [token service](configuring-an-external-key-manager.md)

### Step 1 - Create and deploy an API to Choreo Connect via API Manager
Follow the instructions in the guide [Deploying a REST API in Choreo Connect](../../deploy-api/deploy-rest-api-in-choreo-connect.md).
