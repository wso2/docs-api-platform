...
The Callout mediator is not affected by the [DISABLE_CHUNKING property](property-reference/http-transport-properties.md). Instead, you can disable chunking for the Callout mediator by setting the following parameters in the `MI_HOME/conf/deployment.toml` file:
