...
This feature is only supported for the [Regular Gateway and the Request flow](../../api-design-manage/design/api-policies/overview.md). In order to enable this feature for the JWTs, two conditions need to be fulfilled.
...
This step can be done by following the steps listed [here](../../api-design-manage/design/api-policies/attach-policy.md). The below diagram shows which policy to select. You can apply the policy to the whole API or a designated API resource depending on the use case. 
...
The second step is to add the scope that was defined in the scope attribute of the ‘Revoke One Time Token’ policy. Further information on adding scope to JWTs can be found [here](../../api-security/runtime/authorization/oauth2-scopes/fine-grained-access-control-with-oauth-scopes.md). 