...
For information, see [Installing and Configuring the Databases]({BASE_URL}/api-manager/4.7.0/install-and-setup/setup/setting-up-databases/overview/).
...
For more information, see [Production Deployment Guidelines]({BASE_URL}/api-manager/4.7.0/install-and-setup/setup/deployment-best-practices/production-deployment-guidelines/#production-deployment-checklist).

!!! Note
    Configure the same encryption key on all API Manager instances before starting any node for the first time so they can decrypt shared registry resources consistently. For more information, see [Configuring Encryption Key]({BASE_URL}/api-manager/4.7.0/install-and-setup/setup/security/encryption/symmetric-encryption/#generate-a-secret-key).
...
For more information, see [Creating SSL Certificates]({BASE_URL}/api-manager/4.7.0/install-and-setup/setup/security/configuring-keystores/keystore-basics/creating-new-keystores/).
...
See the instructions on [configuring the Gateway]({BASE_URL}/api-manager/4.7.0/monitoring/api-analytics/moesif-analytics/moesif-integration-guide/) with the cloud-based analytics solution.
...
3. If required, encrypt the Auth Keys (access tokens, client secrets, and authorization codes), see [Encrypting OAuth Keys]({BASE_URL}/api-manager/4.7.0/api-security/key-management/tokens/encrypting-oauth2-tokens/).
...
4. Follow the steps given below to configure High Availability (HA) for the Key Manager nodes:
    ...
    2. Configure a load balancer fronting the two Key Manager nodes in your deployment. For instructions, see [Configuring the Proxy Server and the Load Balancer]({BASE_URL}/api-manager/4.7.0/install-and-setup/setup/setting-up-proxy-server-and-the-load-balancer/configuring-the-proxy-server-and-the-load-balancer/).
...
4. Add the following configuration to set a unique identifier for each Gateway node when setting up a distributed deployment on VMs.
    ...
    !!! Info
        To further optimize the gateway notification feature, you can use additional `gateway_notification` configurations. For more information, see [API-M Revision Deployment Monitoring]({BASE_URL}/api-manager/4.7.0/reference/config-catalog/#api-m-gateway-notification-configurations).

5. Enable JSON Web Token (JWT) if required. For instructions, see [Generating JSON Web Token]({BASE_URL}/api-manager/4.7.0/api-gateway/passing-enduser-attributes-to-the-backend-via-api-gateway/).

6. Add the public certificate of the private key (that is used for signing the tokens) to the truststore under the "gateway_certificate_alias" alias. For instructions, see [Create and import SSL certificates]({BASE_URL}/api-manager/4.7.0/install-and-setup/setup/security/configuring-keystores/keystore-basics/creating-new-keystores/).
...
7. Follow the steps given below to configure High Availability (HA) for the Classic Gateway:
    ...
    2. Configure a load balancer fronting the two Gateway nodes in your deployment. For instructions, see [Configuring the Proxy Server and the Load Balancer]({BASE_URL}/api-manager/4.7.0/install-and-setup/setup/setting-up-proxy-server-and-the-load-balancer/configuring-the-proxy-server-and-the-load-balancer/).
...
2. Add the following configurations to the deployment.toml file.
    ...
    **Connecting the API Control Plane to the Gateway node**:
    ...
    !!! Info
        This configuration is used for deploying APIs to the Gateway and for connecting the Developer Portal component to the Gateway if the Gateway is shared across tenants. If the Gateway is not used in multiple tenants, you can create a [Gateway Environment using the Admin Portal]({BASE_URL}/api-manager/4.7.0/api-design-manage/deploy-and-publish/deploy-on-gateway/deploy-api/exposing-apis-via-custom-hostnames/#using-a-new-gateway-environment-to-expose-apis-via-custom-hostnames).  
...
5. Follow the steps given below to configure High Availability (HA) for the API Control Plane:
    ...
    2. Configure a load balancer fronting the two API Control Plane nodes in your deployment. For instructions, see [Configuring the Proxy Server and the Load Balancer]({BASE_URL}/api-manager/4.7.0/install-and-setup/setup/setting-up-proxy-server-and-the-load-balancer/configuring-the-proxy-server-and-the-load-balancer/).
