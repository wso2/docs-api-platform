...
4. Select **API Key**. Note that `APIKey` is used as the default header. This value can be changed using the `APIKey Header` field.
     ...
     [![Configure API key authentication]({BASE_URL}/api-manager/4.7.0/assets/img/learn/api-key-option.png)]({BASE_URL}/api-manager/4.7.0/assets/img/learn/api-key-option.png)
