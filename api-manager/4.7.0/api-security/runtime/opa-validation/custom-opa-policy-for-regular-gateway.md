---
title: "Create a Custom OPA Policy for the Regular Gateway"
description: "Build a custom Open Policy Agent (OPA) request generator for the regular gateway by implementing the OPARequestGenerator interface."
canonical_url: https://wso2.com/api-platform/docs/api-manager/4.7.0/api-security/runtime/opa-validation/custom-opa-policy-for-regular-gateway/
md_url: https://wso2.com/api-platform/docs/api-manager/4.7.0/api-security/runtime/opa-validation/custom-opa-policy-for-regular-gateway.md
tags:
  - api-manager
  - api-gateway
  - api-security
...
- Create a Java project (For Apache Maven) with following dependencies.
    ```xml
    <dependencies>
        <dependency>
...
- Build the project and create the JAR file. For Apache Maven, use the following command.
    ```bash
    mvn clean install
    ```

- Add the JAR file containing the Custom OPA Request Generator to the `<APIM_HOME>/repository/components/libs` folder.

- Create a new policy from the policies window with the following policy definition. Make sure that you define the policy attributes `serverUrl`, `policy` and `rule` from the create new policy form. 
