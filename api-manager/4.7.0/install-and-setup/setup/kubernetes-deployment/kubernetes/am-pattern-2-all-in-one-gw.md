...
description: "Deploy WSO2 API Manager on Kubernetes with a dedicated Classic Gateway alongside the All-in-One node so the Gateway can scale independently."
...
All configurations in this section are made by editing your `values-aio.yaml` or `values-gw.yaml` files.
