...
description: "Start WSO2 API Manager on a selected profile so only the relevant components load, either before or during server startup."
canonical_url: https://wso2.com/api-platform/docs/api-manager/3.2.0/install-and-setup/setup/distributed-deployment/product-profiles/
md_url: https://wso2.com/api-platform/docs/api-manager/3.2.0/install-and-setup/setup/distributed-deployment/product-profiles.md
tags:
  - api-manager
  - install-and-setup
  - setup
...
!!! note
    Before running these profiles, take a backup of the `<PRODUCT-HOME>/repository/resources/conf/templates/repository/conf/axis2` directory and the `<PRODUCT-HOME>/repository/resources/conf/templates/repository/conf/registry_TM.xml.j2` file.
...
!!! info "If you update the product prior to running the profile"

    If you updated the prodoct prior to running the above command and starting the profile, you may encounter an issue relevant to loading the axis2 configuration. In the event that you encounter this, check if all the files are available in the `<PRODUCT-HOME>/repository/resources/conf/templates/repository/conf/axis2` directory and if it tallies with the backup directory. If not, please add the missing J2 files accordingly. Do the same for the `<PRODUCT-HOME>/repository/resources/conf/templates/repository/conf/registry_TM.xml.j2` file and compare it against the backup you took earlier. Once the template files are in place, run the profile optimization tool again.
