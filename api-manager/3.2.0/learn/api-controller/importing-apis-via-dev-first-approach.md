...
4. Open the `<API Project>/Meta-information/api.yaml` file. You can edit the mandatory configurations listed below.
    ...
    For more information about the configurations, see the [Sample_Api.yaml](https://github.com/wso2/product-apim-tooling/blob/3.2.x/import-export-cli/box/resources/init/sample_api.yaml).
...
!!! tip
    A user with `admin` role is allowed to import APIs. To create a custom user who can import APIs, refer [Steps to Create a Custom User who can Perform API Controller Operations](advanced-topics/creating-custom-users-to-perform-api-controller-operations.md).
