---
title: "Call mediator"
description: "Reference for the Call mediator, which sends messages out of the Micro Integrator to an endpoint in blocking or non-blocking mode."
canonical_url: https://wso2.com/api-platform/docs/api-manager/4.2.0/reference/mediators/call-mediator/
md_url: https://wso2.com/api-platform/docs/api-manager/4.2.0/reference/mediators/call-mediator.md
tags:
  - api-manager
  - reference
  - mediators
...
very much similar to the [Callout mediator](../../reference/mediators/callout-mediator).
...
!!! Info
    The Call mediator is a [conditionally content aware](about-mediators#classification-of-mediators).
...
In this example, the Call mediator invokes a backend service. An [Enrich mediator](../../reference/mediators/enrich-mediator) stores the response received for
that service invocation.

The [Filter Mediator](../../reference/mediators/filter-mediator) added after the Call mediator
...
first service is retrieved by the [Enrich mediator](../../reference/mediators/enrich-mediator)
from the property where it was formerly stored. This response is sent to
the client by the [Respond mediator](../../reference/mediators/respond-mediator).
...
In this example, the message will be cloned by the [Clone Mediator](../../reference/mediators/clone-mediator) and sent via the Call mediator. The Drop mediator drops the response so that no further mediation is carried out for the cloned message. However, since the `         continueParent        ` attribute of the [Clone mediator](../../reference/mediators/clone-mediator) is set to `         true        ` , the original message is mediated in parallel. Therefore, the [Log Mediator](../../reference/mediators/log-mediator) at the end of the configuration will log the `         After call mediator        ` log message without waiting for
...
In the following sample configuration, the [Header Mediator](../../reference/mediators/header-mediator) is used to add the action, the [PayloadFactory Mediator](payloadfactory-mediator) is used to store the the request message and the Call mediator is used to invoke a backend service. You will see that the payload of the request and header action are sent to the backend. After successful backend service invocation, you will see that the response of the service is retrieved by the Micro Integrator and sent to the client as the response using the [Respond Mediator](../../reference/mediators/respond-mediator).
