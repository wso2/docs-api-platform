...
description: "Explains how to debug a mediation flow using instant debugging in Integration Studio or an external Micro Integrator server."
canonical_url: https://wso2.com/api-platform/docs/api-manager/4.2.0/integrate/develop/debugging-mediation/
md_url: https://wso2.com/api-platform/docs/api-manager/4.2.0/integrate/develop/debugging-mediation.md
tags:
  - api-manager
  - integrate
  - develop
...
1.  When project artifacts are ready, select the project you want to debug and click **Run** -> **Debug**.
    ...
    ![select debugging](../../assets/img/integrate/mediation-debugging/debugging-1.png)
...
2.  It will ask to choose the artifacts those needs to be deployed to the embedded Micro Integrator. Internally WSO2 Integration Studio will generate a CAR application with chosen artifacts and deploy.  
    ![select debugging](../../assets/img/integrate/mediation-debugging/debugging-2.png)
...
4.  Add some breakpoints in the flow as below. You can mark a particular
    ...
    ![select debugging](../../assets/img/integrate/mediation-debugging/debugging-3.png)
5.  Invoke the service using SOAP UI or some external client. As soon as
    ...
    ![select debugging](../../assets/img/integrate/mediation-debugging/debugging-4.png)
...
6.  Click **Continue**. Then the message will be sent to
    ...
    will be triggered.  
    ![select debugging](../../assets/img/integrate/mediation-debugging/debugging-5.png)
...
7.  Click **Continue** again. Response will be received by the client.  
    ![select debugging](../../assets/img/integrate/mediation-debugging/debugging-6.png)
...
1.  Click **Run** in the top menu of the WSO2 Integration Studio, and
    ...
    ![select debugging](../../assets/img/integrate/mediation-debugging/debugging-7.png)
2.  Enter the details to create a new configuration as shown in the
    ...
    ![select debugging](../../assets/img/integrate/mediation-debugging/debugging-8.png)
      
3.  Add the new configuration to the Debug menu. Then you can access the configuration easily.  
    ![select debugging](../../assets/img/integrate/mediation-debugging/debugging-9.png)
...
5.  Click the **downward** arrow beside **Debug** in WSO2 Integration Studio and select the new profile created above when the Console indicates the following.
    ...
    ![select debugging](../../assets/img/integrate/mediation-debugging/debugging-10.png) 
    ![select debugging](../../assets/img/integrate/mediation-debugging/debugging-11.png)
...
6.  In WSO2 Integration Studio, right-click and add breakpoints or skip points on the desired mediators to start debugging as shown in the example below.
    ...
    ![select debugging](../../assets/img/integrate/mediation-debugging/debugging-12.png)
    ...
    !!! Info
        ...
        -   **Toggle Skip Point:** Adds a skip point to the selected
            mediator
        -   **Resend Debug Points:** If you re-start the Micro Integrator, or if you re-deploy the proxy service after changing its Synapse configuration, you need to re-send the information on breakpoints to the Micro Integrator server. This re-sends all registered debugging points to the server.
...
![select debugging](../../assets/img/integrate/mediation-debugging/debugging-13.png)

You can view the message payload at that point of the message flow also in the **Message Envelope** tab as shown below.

![select debugging](../../assets/img/integrate/mediation-debugging/debugging-14.png) 
...
![select debugging](../../assets/img/integrate/mediation-debugging/debugging-15.png)
...
![select debugging](../../assets/img/integrate/mediation-debugging/debugging-16.png)
...
1.  Right click on the **Variable** view, click **Inject/Clear Property**, and then click **Inject Property** as shown below.  
    ![select debugging](../../assets/img/integrate/mediation-debugging/debugging-17.png)
2.  Enter the details about the property you prefer to add as shown in the example below.  
    ![select debugging](../../assets/img/integrate/mediation-debugging/debugging-18.png)
3.  Click **OK**.
4.  When the next debug point is hit, you will see the property is set to the specified context.  
    ![select debugging](../../assets/img/integrate/mediation-debugging/debugging-19.png)
...
1.  Right click on the **Variable** view, click **Inject/Clear
    ...
    ![select debugging](../../assets/img/integrate/mediation-debugging/debugging-20.png)
2.  Enter the details about the property you want to clear as shown in
    the example below.  
    ![select debugging](../../assets/img/integrate/mediation-debugging/debugging-21.png)
3.  Click **OK** .
...
1.  Click on the value section of the preferred property and change the value in the **Variable** view as shown in the example below, to modify it.  
    ![select debugging](../../assets/img/integrate/mediation-debugging/debugging-22.png) 
2.  You will see that the property is changed on the property view.  
    ![select debugging](../../assets/img/integrate/mediation-debugging/debugging-23.png) 