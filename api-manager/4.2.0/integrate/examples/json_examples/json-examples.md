...
Note that we have used the [Property mediator](../../../reference/mediators/property-mediator) to mark the outgoing payload to be formatted as JSON:
...
!!! Info
    If you need to convert complex XML responses (e.g., XML with with `         xsi:type        ` values), you will need to set the message type using the [Property mediator](../../../reference/mediators/property-mediator) as follows:
...
mediator](../../../reference/mediators/log-mediator) as shown
...
The [PayloadFactory mediator](../../../reference/mediators/payloadfactory-mediator) provides the simplest way to work with JSON payloads. Suppose we have a service that returns the following response for a search query:
...
You can use the [Validate mediator](../../../reference/mediators/validate-mediator)
...
`         schema\StockQuoteSchema.json        ` ). You do not have any source attributes specified. Therefore, the schema will be used to validate the complete JSON body. The mediation logic to follow if the validation fails is defined within the on-fail element. In this example, the [PayloadFactory mediator](../../../reference/mediators/payloadfactory-mediator) creates a fault to be sent back to the party, which sends the message.
...
```
 ...
 ```
For more instructions on adding loggers, see [Configuring Log4j Properties](../../../observe/micro-integrator/classic-observability-logs/configuring-log4j2-properties).