...
!!! Info
    See the [JMS troubleshooting guide](../../../../troubleshooting/troubleshooting-jms) for more topics related to tuning JMS use cases.
...
This issue occurs when you do not have enough threads available to consume messages from JMS queues. The maximum number of concurrent consumers (that is, the number of JMS proxies) that can be deployed is limited by the base transport worker pool that is used by the [JMS transport](../../../../reference/synapse-properties/transport-parameters/jms-transport-parameters). You can configure the size of this worker pool using the system properties `lst_t_core` and `lst_t_max`. Note that increasing these values will also increase the memory consumption, because the worker pool will allocate more resources.
