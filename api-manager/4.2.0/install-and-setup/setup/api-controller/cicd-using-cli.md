...
description: "Build a CI/CD pipeline with apictl by initializing API projects from OpenAPI specs and exporting, importing, and promoting APIs across environments."
canonical_url: https://wso2.com/api-platform/docs/api-manager/4.2.0/install-and-setup/setup/api-controller/cicd-using-cli/
md_url: https://wso2.com/api-platform/docs/api-manager/4.2.0/install-and-setup/setup/api-controller/cicd-using-cli.md
tags:
  - api-manager
  - install-and-setup
  - setup
...
1.  Download and install WSO2 API-M 4.2.0 in your environments.
     ...
     For more information, see [Installation Prerequisites](../../install/installation-prerequisites).

2.  Download and setup apictl 4.2.7 version. 
...
For this example, let us use the [Swagger Petstore - OpenAPI 3.0](https://petstore3.swagger.io/). 
  ...
  (For more information, see [Create a REST API from an Open API Definition](../../../design/create-api/create-rest-api/create-a-rest-api-from-an-openapi-definition)).
  ...
  7. [Deploy](../../../deploy-and-publish/deploy-on-gateway/deploy-api/deploy-an-api) and [Publish](../../../deploy-and-publish/publish-on-dev-portal/publish-an-api) the API.
...
1.  Sign in to the WSO2 API-M in the lower environment via the apictl.
    ...
    !!! tip
        A user with `Internal/devops` role or `admin` role are allowed to export APIs. To create a custom user who can export APIs, refer [Steps to Create a Custom User who can Perform API Controller Operations](advanced-topics/creating-custom-users-to-perform-api-controller-operations#creating-custom-users-to-perform-apictl-operations).
...
2. Export the latest revision of the API from the lower environment using the `export api` command.
    ...
    !!! example
     ...
     For more information, see [Export an API](managing-apis-api-products/migrating-apis-to-different-environments#export-an-api).
...
[Initialize an API Project](managing-apis-api-products/importing-apis-via-dev-first-approach#initialize-an-api-project).
...
Create two (2) repositories in your Version Control System. You can give any names to these repositories, but in this example we will be naming those as **Source** and **Deployment**. Note that, **Deployment** repository is only needed if you want to [Configure Environment Specific Parameters](advanced-topics/configuring-environment-specific-parameters).
...
2. Follow the below steps if you want to define the environment-specific details for the API.
    ...
    2. Open the `params.yaml` inside the `DeploymentArtifacts_SwaggerPetstore-1.0.0` in a text editor and define the *prod.wso2.com* and *prod.sandbox.wso2.com* as the backend URLs.
        ...
        !!! info
            ...
            - For more information on the parameters file of an API, see [Defining the parameters file for an API](advanced-topics/configuring-environment-specific-parameters#defining-the-parameters-file-for-an-api).
            - For more information on the parameters file of an API Products, see [Defining the parameters file for an API Product](advanced-topics/configuring-environment-specific-parameters#defining-the-parameters-file-for-an-api-product).
            - The apictl supports detecting environment variables defined. If an environment variable is not set, the tool will fail. Also, the system will request the user for a set of required environment variables to ensure that information is not missing during the migration process. For more information, see [Using Dynamic Data in apictl Projects](advanced-topics/using-dynamic-data-in-api-controller-projects).
...
6.  Import the **SwaggerPetstore** API into the production environment by running the following sample command.
    ...
    !!! tip
        ...
        - A user with `Internal/devops` role or `admin` role are allowed to import APIs. To create a custom user who can import APIs, see [Steps to Create a Custom User who can Perform API Controller Operations](advanced-topics/creating-custom-users-to-perform-api-controller-operations#minimal-permissions-and-scopes-required-to-perform-apictl-operations).
...
1.  Import the **SwaggerPetstore** API into the production environment and test the API by running the following sample command.
    ...
    !!! note
        ...
        - For more information on importing an API to an environment, see [Import an API](managing-apis-api-products/migrating-apis-to-different-environments#import-an-api).
...
!!! info  
    ...
    -   For example, if an API is in the `PUBLISHED` state in the development environment, it will also be in the same state in the testing environment. This default behavior can be changed via the apictl tool, which assigns APIs the `CREATED` state after importing. See [Change status of an API or API Product in an environment](managing-apis-api-products/managing-apis-and-api-products#change-status-of-an-api-or-api-product-in-an-environment) for more information.

<a name="Step 7"></a>
### Step 7 - Get keys for an API/API Product

Follow the instructions below to generate a JWT/OAuth token for testing purposes using apictl in order to invoke an API or an [API Product](../../../design/create-api-product/api-product-overview) by subscribing to it using a new application created by apictl.

!!! tip
    - Make sure that WSO2 API-M is started and the apictl is set up. For more information, see [Download and Initialize the apictl](getting-started-with-wso2-api-controller#download-and-initialize-the-apictl). 
    - You should log in to the WSO2 API-M in the environment by following the instructions in [Login to an Environment](getting-started-with-wso2-api-controller#login-to-an-environment).
...
!!! info
    ...
    - Using apictl tool the HTTP request timeout, and export directory can be set up and changed. For more information on changing the HTTP request timeout, see [Set HTTP request timeout](getting-started-with-wso2-api-controller#set-http-request-timeout) and [Set export directory](getting-started-with-wso2-api-controller#set-export-directory) accordingly. 
...
For example, let us consider there is an [API Product](../../../design/create-api-product/api-product-overview) **PetsInfo** in the development environment with a subset of operations of **SwaggerPetstore** API.

1.  Export the API Product using `export api-product` command from the development environment (dev). For more information, see [Export an API Product](managing-apis-api-products/migrating-api-products-to-different-environments#export-an-api-product).
