...
description: "Take heap dumps from the Choreo Connect Adapter, Enforcer, and Router components to diagnose memory resource exhaustion issues."
canonical_url: https://wso2.com/api-platform/docs/api-manager/4.2.0/deploy-and-publish/deploy-on-gateway/choreo-connect/troubleshooting/debug-memory/
md_url: https://wso2.com/api-platform/docs/api-manager/4.2.0/deploy-and-publish/deploy-on-gateway/choreo-connect/troubleshooting/debug-memory.md
tags:
  - api-manager
  - deploy-and-publish
  - deploy-on-gateway
...
2.  Build ***Router Debug Image***, if you are taking heap profile **on top of an updated Router image**.
    ...
    !!! note
        For the **GA version** of the Router Docker image, you can use `wso2am/choreo-connect-router:1.2.0-debug-v1.24.1` as the **Router Debug Image**.
    ...
    1.  Create a Dockerfile as follows. Here we create a custom Docker image by replacing the envoy binary in default Docker image with heap profile enabled envoy binary.
        ...
        === "Format"
            ...
            COPY --from=wso2am/choreo-connect-router:1.2.0-debug-v1.24.1 /usr/local/bin/envoy /usr/local/bin/envoy
            ```

        === "Sample Dockerfile"
            ```Dockerfile
            FROM wso2/choreo-connect-router:1.2.0.1 # for update level 1
            COPY --from=wso2am/choreo-connect-router:1.2.0-debug-v1.24.1 /usr/local/bin/envoy /usr/local/bin/envoy
            ```
    ...
    2. Build a Docker image and push it to a Docker registry that your Kubernetes cluster can pull it.
        ...
        === "Sample"
            ```bash
            docker build -t sample.com/choreo-connect-router:1.2.0.1-debug .
            ```
...
1.  Change the Router Docker image with the built ***Router Debug Image*** and mount a directory to `/var/log/envoy` to collect head profile data.
    ...
    === "K8s Helm"
        ```bash
        helm upgrade --install <RELEASE_NAME> wso2/choreo-connect --version 1.2.0-1 --namespace <NAMESPACE> \
