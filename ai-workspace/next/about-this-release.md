...
??? note "LLM providers for seven AI services"
    ...
    - **Inbound authentication with API keys**: The gateway checks an API key on every incoming client request to a deployed provider. The workspace generates each key, shows it once, and sets a 90-day validity period. By default, clients use the vendor's header, allowing the vendor's software development kit (SDK) to work unchanged. The [inbound authentication configuration](configure-inbound-auth.md) lets you change this header in the provider's **Security** tab. Inbound keys are separate from the upstream API key the gateway uses to call the AI service.
