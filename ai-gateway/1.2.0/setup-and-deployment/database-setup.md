...
canonical_url: https://wso2.com/api-platform/docs/ai-gateway/1.2.0/setup-and-deployment/database-setup/
md_url: https://wso2.com/api-platform/docs/ai-gateway/1.2.0/setup-and-deployment/database-setup.md
tags:
  - ai-gateway
  - configuration
  - postgresql
...
=== "SQL Server"
    ...
    database = "gateway_controller"
    user = "gateway"
    password = '{{ env "APIP_GW_CONTROLLER_STORAGE_DATABASE_PASSWORD" "" }}'
...
The `{{ env "..." "" }}` form is the interpolation token already used in the shipped `config.toml` — it reads the value from an environment variable at load time instead of storing it in the file. Set the actual values in `api-platform.env`:
...
port = 6379
password = '{{ env "APIP_GW_RATELIMIT_REDIS_PASSWORD" "" }}'
```

`config.toml` is interpolated the same way everywhere in the file, so the `{{ env "..." "" }}` token above works here too — see [Gateway Configuration and Environment Interpolation](./configuration.md) rather than writing the password literally.
